...
class BusProcessor : AbstractProcessor() {
    ...
    override fun getSupportedAnnotationTypes() = setOf(BusEvent::class.java.canonicalName, AutoWire::class.java.canonicalName, Service::class.java.canonicalName)
    ...
    private fun fetchAutoWireInfo(e: Element) {
        ...
        if (e is VariableElement) {
            ...
            info.autoWire.add(AutoWireInfo(e.simpleName.toString(), Utils.getSignature(e), FUNCTION))
        }
    ...
    private fun typeMete(): MethodSpec {
        ...
        val p = ParameterizedTypeName.get(ClassName.get(HashMap::class.java), stringType, ClassName.get(TypeMete::class.java))
        ...
        for ((k, v) in typeMete) {
            method.addStatement("map.put(\$S,new \$T(${v.createStrategy},${v.isService},\$S))", k, typeMeteType, v.canProvideFrom)
        }
        return method.build()
    }

    private fun eventAware(info: BusAwareInfo): MethodSpec {
        val p = ParameterizedTypeName.get(ClassName.get(List::class.java), WildcardTypeName.subtypeOf(ClassName.get(Any::class.java)))
        ...
        for (event in info.busEvent) {
            val types = Utils.getFieldFromSignature(event.argsSignature).map { Utils.getClassName(it) }.toTypedArray()
            if (types.size > 1) {
    ...
    private fun createMethod(info: BusAwareInfo): MethodSpec {
        ...
        if (signature.isNullOrEmpty()) {
            ...
            val typesName = Utils.getFieldFromSignature(signature)
            val types = typesName.map { Utils.getClassName(it) }.toTypedArray()
            creatorMethod.addStatement("Object args[] = new Object[${types.size}]")
            for (index in typesName.indices) {
                creatorMethod.addStatement("args[$index]= fetcher.fetch(\$S)", typesName[index])
            }
            val builder = StringBuilder()
            builder.append("if(")
            for (index in types.indices) {
                builder.append("args[$index] instanceof \$T &&")
            }
            builder.deleteCharAt(builder.length - 1)
            builder.deleteCharAt(builder.length - 1)
            builder.append(")")
            creatorMethod.beginControlFlow(builder.toString(), *types)
            builder.delete(0, builder.length)
            builder.append("instance = new \$T(")
            for (index in types.indices) {
                builder.append("(\$T)args[$index],")
            }
            builder.deleteCharAt(builder.length - 1)
            builder.append(")")
            creatorMethod.addStatement(builder.toString(), instanceType, *types)
                    .endControlFlow()
            val e = ClassName.get(RuntimeException::class.java)
            creatorMethod.beginControlFlow("else")
                    .addStatement("throw new \$T(\$S)",e,"无法创建此类${instanceType.canonicalName()}，非空参数不足,\n所需参数不能在本类或者创建它的类中@provide获得")
                    .endControlFlow()
        }
    ...
    private fun creatorHandler(info: BusAwareInfo) {
        ...
        for (provide in info.provideInfo) {
            provideFun.beginControlFlow("if(\$T.equals(returnTypeName,\$S))", objectsType, provide.returnType)
        ...
        for (autoWire in info.autoWire) {
            val (pak, name) = Utils.getClassNameFromPath(autoWire.argsSignature)
            val type = ClassName.get(pak, name)
            val returnName = "${autoWire.name}_fun"
            autoWireMethod.addStatement("Object $returnName = fetcher.fetch(\$S)", autoWire.argsSignature)
            if (autoWire.wireType == FIELD) {
                autoWireMethod.beginControlFlow("if($returnName != null)")
                        .addStatement("instance.${autoWire.name} = (\$T)$returnName", type)
                        .endControlFlow()
        ...
        val type = TypeSpec.classBuilder(typeName)
                ...
                .addMethod(autoWireMethod.build())
                .addMethod(cons)
    ...
    private fun generateFromBus() {
        val fetcherMethod = MethodSpec.methodBuilder("fetch")
                ...
                .addParameter(ParameterSpec.builder(ClassName.get(Fetcher::class.java), "fetcher").build())
                .addParameter(ParameterSpec.builder(ClassName.get(Any::class.java), "receiver").build())
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override::class.java)
